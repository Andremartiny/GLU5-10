...
1. For å bruke figurtall-som-sum er vi nødt til å fremheve tilleggene fra figur til figur. Gjør dette selv ved å tegne! Se også \
...
1. Under ser du figurtall én til fire. Finn eksplisitt sammenheng ved hjelp av strategien *figurtall-som-sum-knepet*.
...
$F_n = 8n + n(n+1)+2(n-1) = 8n+n^2 + n+2n-2 = n^2 + 11n-2$.
...
På figuren under ser du de tre første figurene i en sammensatt figur, der $F_1 = 9$, $F_2 = 18$ og $F_3 = 30$. Finn en rekursiv formel ved hjelp av strategien form på tillegg.

![](https://raw.githubusercontent.com/Andremartiny/MA-173/1771a18e040735ec0055318ae74c13530a161541/img/tallteo/b%C3%A5ttall.drawio.svg) <br> <br> 

##### **Vurderingskriterier**

Besvarelsen *må* bruke strategien form på tillegg.

For eksempel kan dette gjøres slik. Kikker vi geometrisk kan vi se tre trekanter og en rad, slik vist i figuren. <br> ![](https://raw.githubusercontent.com/Andremartiny/MA-173/1771a18e040735ec0055318ae74c13530a161541/img/tallteo/b%C3%A5ttall2.drawio.svg) <br> Den røde trekant vokser i takt med figurtallsnummeret, og øker dermed med $n$ fra $F_{n-1}$ til $F_n$. Den blå og lilla trekanten øker med $n+1$ fra $F_{n-1}$ til $F_n$. Raden øker bare med én hver gang. Det gir totalt at $F_{n} = F_{n-1} + n+2(n+1)+1 = 3n+3 =3(n+1)$. I tillegg får vi vite at $F_1 = 9$.  

#### 1.9 Finne rekursivt uttrykk ved hjelp av strategien differanse mellom eksplisitte uttrykk

##### Medium: Finne rekursivt uttrykk for trekant-, rektangel- og kvadrattall

#### Øveoppgaver

1. Finn rekursivt uttrykk for trekanttallene $T_n$, der $T_1 =1$, $T_2 = 3$ og $T_3 = 6$ ved hjelp av strategien differanse mellom eksplisitte uttrykk. Merk: Det eksplisitte uttrykket for trekanttallene er $T_n = \frac{{n(n+1)}}{2}$.
2. Finn rekursivt uttrykk for kvadrattallene $K_n$, der $K_1 =1$, $K_2 = 4$ og $K_3 = 9$ ved hjelp av strategien differanse mellom eksplisitte uttrykk. Merk: Det eksplisitte uttrykket for kvadrattallene er $K_n = n^2$
3. Finn rekursivt uttrykk for rektangeltallene $R_n$, der $R_1 =2$, $R_2 = 6$ og $R_3 = 12$ ved hjelp av strategien differanse mellom eksplisitte uttrykk. Merk: Det eksplisitte uttrykket for rektangeltallene er $R_n = n(n+1)$.
...
1. Finn rekursivt uttrykk for sekskanttallene $H_n$, der $H_1 =1$, $H_2 = 6$ og $H_3 = 15$ ved hjelp av strategien differanse mellom eksplisitte uttrykk. Merk: Det eksplisitte uttrykket for sekskanttallene er $H_n = {n(2n-1)}$.
2. Illustrer femkanttallene opp til $F_3$, og utled rekursivt uttrykk for $F_n$ ved hjelp av strategien differanse mellom eksplisitte uttrykk. Merk at tallrekken som tilhører femkanttallene er $1, 5, 12, 22, \ldots$ og at det eksplisitte uttrykket for sekskanttallene er $F_n = \frac{{3n^2-n}}{2}$.
3. Illustrer syvkanttallene opp til $S_3$, og utled rekursivt uttrykk for $S_n$ ved hjelp av strategien differanse mellom eksplisitte uttrykk. Merk at tallrekken som tilhører syvkanttallene er $1, 7, 18, 34, \ldots$. og at det eksplisitte uttrykket for sekskanttallene er $S_n = \frac{{5n^2-3n}}{2}$.
...
Over ser du de tre første figurene i bikubetallene. F
inn rekursivt uttrykk for bikubetallene $B_n$, der $B_1 =1$, $H_2 = 7$ og $H_3 = 19$ ved hjelp av strategien differanse mellom eksplisitte uttrykk. Merk: Det eksplisitte uttrykket for bikubetallene er $B_n = {3n(n-1)}+1$.
...
Et figurtallsmønster kan beskrives eksplisitt ved formelen $F_n = (n+1)(2n+3)$. Finn et rekursivt uttrykk for figurene ved hjelp av differanse mellom eksplisitte uttrykk.
...
#### 1.10 Beskrive oppbygningen av figurtall

##### Mild: Beskrive eksplisitt og rekursiv sammenheng med ord, og vise i figuren.

1. Se på figurtallene under.
    a. Gi en så kort og presis verbal beskrivelse du kan av hvordan figurene vokser (rekursiv sammenheng).
    b. Illustrer den rekursive sammenhengen i figurene.
    c. Gi en beskrivelse av den eksplisitte sammenhengen mellom figurnummer og figurtall (presisjon er målet her også).
...
1. a. Dette kan gjøres på flere måter. For eksempel kan en se forrige figur i neste, og peke på hva som er lagt til for å bygge den nye figuren. Vi kan også peke på at figuren består av to deler. En trekant, som står på et rektangel. For å lage neste figur så skyv trekanten opp og legg til en bunn på størrelse $n+1$. Rektangelet skal øke én i høyde og én i bredde også. Dermed må vi legge til $n$ og $n-1$. Totalt legger vi altså til $3n$. <br>b. Under er sammenhengen markert. De grønne er forrige figur, det røde er det som legges til i trekanten, det blå er bunnen som legges til i rektangelet, og det gule er siden som legges til i rektangelet. <br> ![Rekursiv sammenheng](https://raw.githubusercontent.com/Andremartiny/MA-173/main/img/tallteo/rekursiv.svg) <br>c. Vi kan nå bruke dekomponeringen vi har brukt til å beskrive den rekursive sammenhengen. For figur $P_n$ har vi trekanttall $n+1$ minus toppen ($-1$). Det gir $T_{n+1}-1 = \frac{(n+1)(n+2)}{2}-1$. Rektangelet er alltid $n(n-1)$. <br>d. Vi får at den eksplisitte formelen er $\frac{(n+1)(n+2)}{2}-1+n(n-1)$.
...
![](https://raw.githubusercontent.com/Andremartiny/MA-173/dd6a409136aa6823c4aafc266271816393066d45/img/tallteo/fig17.drawio.svg)
...
![](https://raw.githubusercontent.com/Andremartiny/MA-173/dd6a409136aa6823c4aafc266271816393066d45/img/tallteo/fig17dekomp1.drawio.svg)
...
![](https://raw.githubusercontent.com/Andremartiny/MA-173/b15fcbca3358196bef93bb019492be4cadf6dbe8/img/tallteo/figtallsneg.drawio.svg)

##### Vurderingskriterier

Besvarelsen må illustrere og markere i figuren og svare på *begge* oppgavene. For eksempel

1. Figuren består av tre tydelige deler. En ramme av et kvadrat som er én større enn figurtallsnummeret og to trekanter, én som følger figurtallsnummeret og én som ligger én bak. Rammen er alltid fire ganger figurtallsnummeret og trekantene kan legges sammen for å lage et kvadrat som er like stort som figurtallsnummeret. Legger vi disse sammen finner vi antall firkanter i figurene. Se figuren under.
2. Begynn med et kvadrat som er 2x2 med en blokk midt på toppen av kvadratet. For å lage neste figur skal man gjøre kvadratet én større, men kun beholde rammen til kvadratet. Blokken på toppen (som blir en trekant) skal dyttes én opp og det skal legges en ny rad under som er like lang som figurtallsnummeret. Det skal også legges til en kolonne nederst på høyre side (som skal være en del av en trekant). Kolonnen skal alltid starte nederst til høyre og være én lavere enn figurtallsnummeret. Se figuren under. 

![](https://raw.githubusercontent.com/Andremartiny/MA-173/b15fcbca3358196bef93bb019492be4cadf6dbe8/img/tallteo/figtallsnegdekomp.drawio.svg)
...
![](https://raw.githubusercontent.com/Andremartiny/MA-173/b15fcbca3358196bef93bb019492be4cadf6dbe8/img/tallteo/figtallrosa.drawio.svg)
...
2. Begynn med to firkanter på siden av hverandre, disse skal være starten av to trekanter. 

![](https://raw.githubusercontent.com/Andremartiny/MA-173/b15fcbca3358196bef93bb019492be4cadf6dbe8/img/tallteo/figtallrosadekomp.drawio.svg)
...
1. en eksplisitt sammenheng mellom figur nummer $n$ og antall prikker i figuren.
2. en rekursiv sammenheng mellom to påfølgende figurer.
![](https://raw.githubusercontent.com/Andremartiny/MA-173/590df0ce2d32f13f5465534d4ca3af13eda4363b/img/tallteo/figuirta%C3%B8%C3%B8.drawio.svg) <br>

##### **Vurderingskriterier**

En måte kan være å legge merke til at figuren bare er et deformert kvadrat. Da kan man enkelt se at sammenhengen må være

1. $F_n =(n+2)^2$
2. $F_n = F_{n-1}+2(n+2)-1 =F_{n-1}+2n+3$, der $F_1 = 9$.
<br>
![](https://raw.githubusercontent.com/Andremartiny/MA-173/590df0ce2d32f13f5465534d4ca3af13eda4363b/img/tallteo/figurt%C3%B8l%C3%B8l%C3%B8.drawio.svg)
<br>
...
2. Under ser du trekantramme nummer fire, samt fem forslag til eksplisitt uttrykk for trekantramme nummer $n$. <br>a. Hvordan ser de foregående rammene ut? <br>b. Hvilke uttrykk stemmer? Argumenter ved hjelp av figuren og ved å omforme uttrykkene. <br>![](https://raw.githubusercontent.com/Andremartiny/MA-173/main/img/2023-04-03-20-19-18.png) <br> i. $3(n - 1) + 3$ <br> ii. $(n - 1) + n + (n + 1)$<br> iii. $3n$<br> iv. $\frac{(n + 1)(n + 2)}{2} - \frac{(n - 2)(n - 1)}{2}$ <br> v. $3(n + 1) - 3$

##### Løsningsforslag

1. Noen forslag kan være: Kvadrat og linje og trekant uten topp. Tre trekanter der én mangler første bit. Ett rektangel i bunnen og et halv parallellogram i toppen. Let etter egne og forsøk å pek på forslagene over selv.
2. <br>a. Vi kan se for oss at dette er slik trekantrammene utvikler seg
...
![](https://raw.githubusercontent.com/Andremartiny/MA-173/31b1dc0629a975ed380203490b26e67c131bca4f/img/tallteo/firkanter.drawio.svg)
...
![](https://raw.githubusercontent.com/Andremartiny/MA-173/31b1dc0629a975ed380203490b26e67c131bca4f/img/tallteo/firkanternedbryting.drawio.svg)
...
1. I første utrykk kan vi lese dette som to ledd. Ett kvadrat som er to større enn figurtallsnummeret, der en må trekke vekk to rader som er $n$ lange. I tillegg har vi et trekanttall som mangler én. Se markeringen under. <br> ![](https://raw.githubusercontent.com/Andremartiny/MA-173/dd6a409136aa6823c4aafc266271816393066d45/img/tallteo/fig17dekomp1.drawio.svg)
2. I det andre uttrykket ser vi at vi har fire som er konstant, markert gult under. I tillegg har vi et rektangel som er $n\cdot (n+2)$, markert i rødt under. Til slutt har vi en trekant som mangler én, markert i blått, med skravert firkant som mangler.<br> ![](https://raw.githubusercontent.com/Andremartiny/MA-173/dd6a409136aa6823c4aafc266271816393066d45/img/tallteo/fig17dekomp2.drawio.svg)
3. I det tredje uttrykket har vi et rektangel som er $2n(2n+1)$ stort. Deretter trekker vi fra venstre side på $2n$ (skraverte felter til venstre under), i tillegg trekker vi fra en trekant som mangler én (skraverte felter til høyre under).<br> ![](https://raw.githubusercontent.com/Andremartiny/MA-173/dd6a409136aa6823c4aafc266271816393066d45/img/tallteo/fig17dekomp3.drawio.svg)
4. I det siste uttrykket har vi tre ledd. Ett konstant ledd som er $4$, markert i gult. Et rektangel som er $(n-1)(n+2)$, markert i blått under. Til slutt har vi en trekant som er én større enn figurtallsnummeret. Dette kan vi finne ved å flytte én brikke slik som vist i rødt under.<br>![](https://raw.githubusercontent.com/Andremartiny/MA-173/dd6a409136aa6823c4aafc266271816393066d45/img/tallteo/fig17dekomp4.drawio.svg)
...
![](https://raw.githubusercontent.com/Andremartiny/MA-173/3dd0097091ec32c9673a33a4de186678d2649802/img/tallteo/figurer.drawio.svg)
...
![](https://raw.githubusercontent.com/Andremartiny/MA-173/9ce354c2380ad0fe03387c619b538c56fa22bdea/img/tallteo/dekomfigurer.drawio.svg)
...
![](https://raw.githubusercontent.com/Andremartiny/MA-173/590df0ce2d32f13f5465534d4ca3af13eda4363b/img/tallteo/figuirta%C3%B8%C3%B8.drawio.svg)
<br>

##### **Vurderingskriterier**

I figuren under vises ett eksempel på hvordan man få alle fire uttrykkene til å stemme overens med figurene.

![](https://raw.githubusercontent.com/Andremartiny/MA-173/4f5f5706d41f5956b64d2aedae057b97f3fa5f7e/img/tallteo/figurtall%C3%B8.drawio.svg)
...
3. Den $n$-te figuren i et figurtallmønster har eksplisitt uttrykk $G_{n} = 2n^{2} - (n - 1)^{2} + \frac{n(n + 1)}{2}$.
...
5. Du får vite at et figurtall $F_n$ øker på følgende måte. $F_1 = 2$, $F_2 = 7$, $F_3 = 14$, $F_4 = 23$ og $F_5 = 34$. Lag en figur som følger mønsteret til $F_n$. Begrunn sammenhengen mellom figuren og tallfølgen.
...
3. Under kommer forslag som dere kan bygge videre på, altså ikke rene løsningsforslag
...
![](https://raw.githubusercontent.com/Andremartiny/MA-173/da36800ea95f20e0a43623ccff38fd7709a911bc/img/tallteo/tallf%C3%B8lge2-4-8-14-22.drawio.svg)
...
2. Siden vi nå også har beskrevet tallfølgen vår som to trekanttall bør
...
![](https://raw.githubusercontent.com/Andremartiny/MA-173/87c0d279846a79b216a5af5254e3c88097fee2ca/img/tallteo/lagefrarekursiv.drawio.svg)
...
Besvarelsen må svare på alle oppgavene på en måte som er forståelig og mulig å følge.

For eksempel kan det gjøres slik.
...
![](https://raw.githubusercontent.com/Andremartiny/MA-173/52c8acd9262ff566599688f7fe34f7642dd56aba/img/tallteo/fig1124.drawio.svg)
Nå har vi et utgangspunkt for de resterende oppgavene.

1. Se figuren over med forklaring.
2. Ved å bruke undersøkelsen over ser vi at vi kan legge til $2n+1$ hver gang. Det svarer til å legge til venstre side og bunnen i kvadratet vi har laget. Da overteller vi et hjørne hver gang. Se figuren under som viser den rekursive sammenhengen. <br> ![](https://raw.githubusercontent.com/Andremartiny/MA-173/dd8a608a7b0e7930c183e1bf1539295761ef6915/img/tallteo/fig11233.drawio.svg)
3. Vi har allerede ett eksplisitt uttrykk $F_n = n^2 + 2$. Undersøkelsen viste også at vi nesten har "neste kvadrat", bare at vi mangler én. Det gir altså $F_n = (n+1)^2 -1$, se figuren under.

![](https://raw.githubusercontent.com/Andremartiny/MA-173/84a9eafd9894390006ad3730611d4d737e3f3c57/img/tallteo/fig1123.drawio.svg)
...
Vi undersøker først. Vi kan tolke $2n+1$ på flere måter. Tenker vi trekanttall betyr $2n$ bare at det er to trekanter som øker i tillegg til at $+1$ gir en konstant stigning på én. Dette kan da enkelt tegnes slik (slik at det passer med startverdi) ![](https://raw.githubusercontent.com/Andremartiny/MA-173/84e852cc6984eff8c35e37d2b31ab0c7afdf17c4/img/tallteo/2npluss1rekursivsomtrekant.drawio.svg) <br> Der blåfargen er den ene trekanten, rød er den andre og det hvite er den konstante økningen. <br> Vi kan også merke oss at dette minner om kvadrattallene sin økning, bare at økningen er "neste oddetall i rekka", i tillegg matcher ikke startverdien. Justerer vi litt og lar kun det grønne høre til $F_n$ kan vi uttrykke slik som under. ![](https://raw.githubusercontent.com/Andremartiny/MA-173/84e852cc6984eff8c35e37d2b31ab0c7afdf17c4/img/tallteo/2npluss1somkvardat.drawio.svg)<br>
...
3. Første tolkning over gir $2T_n + n-1 = n(n+1)+n-1$ og andre gir $K_{n+1}=(n+1)^2-2$. Regn ut selv og finn et penere eksplisitt uttrykk. Andre teknikker som figurtall-som-sum, Gauss-triks og stirre hardt kan brukes. 
...
Begrunn hvordan arbeid med figurtall innebærer algebraisk tenkning.

###### Spicy:

Lag en figurtalloppgave, og begrunn hvordan arbeid med den innebærer algebraisk tenkning.
