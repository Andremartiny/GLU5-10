
## Mild

> [!abstract] 1
>  Alfa s. 279
>  1. 3.40 a)--d). Løs likningene på flere måter der det virker naturlig.
>  2. 3.46 Løs likningene på flere måter der det virker naturlig, ikke bare slik boka spør etter.
>  3. 3.47 Løs likningene på flere måter der det virker naturlig, ikke bare slik boka spør etter.

> [!note]- Løsning 
> Alfa s. 279
   3.40 c)
>
> Løsning 1: Utvider til 2-deler, sammenlikner tellerne. Hvis *noe* pluss 10 er null, er *noe* -10.   
> $$
> \begin{align*}
> \frac{3x}{2}+5
> & = \frac{x}{2} \\    \frac{ 3x + 10 }{2}   & = \frac{x}{2}   \\   2x + 10   & = 0   \\   x   & =   -5.   \end{align*}
> $$
> Løsning 2: Sorterer uttrykk med ukjent på VS, tall på HS (trekker fra $\frac{x}{2}$ og $5$ på begge sider av likhetstegnet). To halve er én hel.
> $$
>  \begin{align*}
>  \frac{3x}{2} + 5
>  & =
>  \frac{x}{2}
>  \\
>  \frac{3x}{2}- \frac{x}{2} & = -5   \\   x = -5.  \end{align*}   
>  $$
>   
>   b. Starter i hvert tilfelle med $x$, og leser likninga fra venstre. Deretter rygger vi steg for steg: Starter med *svaret*, og leser (det vil si regner) baklengs med motsatte regneoperasjoner til vi er tilbake på $x$.
>   a) 20. Gange med to (40), trekke fra ti (30), dele på tre (10).
>   b) 15. Trekke fra seks (9), gange med fire (24).
>   c) 10. Gange med tre (30), legge til femten (45), dele på fem (7). \
>  
>  c. Holde over: 
>  
>  a) TELLER delt på to er tjue; TELLER er 40. NOE pluss ti er førti; NOE er 30. Det tredobbelt av tallet er tretti; tallet er 10.
>  b) Noe + 6 = 15, noe = 9. En firedel av noe er 9, noe er 36.
>  c) Teller er 30; 5x er 45; x er 7.
>  d) Noe + 5 = 37; noe = 32. Det dobbelte av noe er 32; noe er 16. Tallet i andre er 16; tallet er 4.
>  e) To ganger noe er 20; noe er 10. $\pi$ ganger tallet er 10; tallet $\frac{10}{\pi}$. .


> [!abstract] 2
> Løs likningen på tre måter: ved hjelp av strategiene _holde over_ og _arbeide baklengs,_ og på «vanlig» måte.
> $$
> \frac{3x + 5}{7} = 2
> $$

> [!note]- Løsning 
> *Holde over*
> Vi holder over nevneren $\displaystyle \frac{\square}{7}= 2$ og ser at $\square$ må være $14$ for å få to ved divisjon av 7. Dermed må $3x+5=14$. Vi holder over og tenker $\triangle+5=14$, vi må altså ha at $\triangle$ er 9, som betyr at $3x=9$. Til slutt holder vi over $x$ og ser at $x=3$. 
> 
> 
> *Arbeide baklengs*
> Vi kan lage denne likningen ved å tenke at følgende skjer med $x$
> $$
> x \xrightarrow{\cdot3} 3x \xrightarrow{+5} 3x+5 \xrightarrow{:7} \frac{3x+5}{7}
> $$
> Effekten av dette er at det produserer $2$. Skal vi reversere får vi
> $$
> 2 \xrightarrow{\cdot7} 14 \xrightarrow{-5} 9 \xrightarrow{:3} 3.
> $$


> [!abstract] 3
> Arne og Berit har $45$ klinkekuler til sammen. Berit har tre flere enn Arne. Hvor mange klinkekuler har hver av dem?
> 1.  Løs ved hjelp av en illustrasjon.
> 2. Løs ved hjelp av en likning.



> [!note]- Løsning 
> 1. 
> 2. Vi kaller $x$ for antall klinkekuler som Arne har. Vi får vite at Berit har tre ganger så mange, altså $x+3$. Totalt har de $x+(x+3)=2x+3$. Videre får vi vite at totalt har de $45$ klinkekuler. Det gir $2x+3=45$ eller $2x=42$, eller $x=21$. Dermed har Arne $21$ og Berit $24$.

> [!abstract] 4
> Arne og Berits samlede alder er $18$. For tre år siden var Berit dobbelt så gammel som Arne. Hvor gamle er hver av dem?
> 1. Løs ved hjelp av en illustrasjon.
> 2. Løs ved hjelp av en likning.


> [!note]- Løsning 
> 1. 
> 2. La $x$ være Arnes alder og $y$ være Berits alder. Vi får vite at $x+y=18$. Vi får også vite at Berit sin alder for tre år siden, $y-3$, var den dobbelte av Arnes alder for tre år siden, $x-3$. Med andre ord er $2(x-3) = y-3$. Skriver vi ut og legger til 3, får vi $2x-6+3=y$ eller at $2x-3=y$. 
>    
>    Setter vi det inn i den originale likningen får vi at $18 = x+y = x+2x-3= 3x-3$. Denne løser vi nå enkelt til $21=3x \longrightarrow x=7$. Det betyr også at $y=2x-3=2\cdot7-3=11$. 



> [!abstract] Vurdering uke 3
> Løs likningen $\frac{3x-5}{2} = 17$. på to måter, formelt og uformelt.

> [!note]- Løsning 
> Likningen må løses både formelt og uformelt.
> Eksempler: Uformelt kan være å jobbe baklengs. Stegene er
> 1. Ta et ukjent tall og gang med 3.
> 2. Trekk fra 5
> 3. Del det nye tallet på 2
> 4. Da får vi 17
> 5. Baklengs: Vi har 17 -> Multipliser med 2 (34)-> Legg til 5 (39)--> Del på tre (13).
> 
> Formelt:
> 
> $$
> \begin{align*}
> \frac{3x-5}{2} & = 17 \quad\quad |\cdot 2\\ 3x -5 & = 34 \quad \quad |+5 \\ 3x & = 39 \quad \quad | :3 \\x & = 13.
> \end{align*}
> $$


> [!abstract] Vurdering uke 4
> Løs likningen $\frac{-2x+5}{3} = 1$. på to måter, formelt og uformelt.

> [!note]- Løsning 
>  **Vurderingskriterier**
>  
>  Likningen må løses både formelt og uformelt.
>  Eksempler: Uformelt kan være å jobbe baklengs. Stegene er
> 
> 1. Ta et ukjent tall og gang med -2.
> 2. Legg til 5
> 3. Del det nye tallet på 3
> 4. Da får vi 1
> 5. Baklengs: Vi har 1 -> Multipliser med 3 (3)-> Trekk fra 5 (-2)--> Del på -2 (1).
> 6. 
> 7. Formelt:
> 
> $$
> \begin{align*}
> \frac{-2x+5}{3} & = 1 & |\phantom{-}\cdot 3\\ -2x +5 & = 3 & |\phantom{\cdot}-5 \\ -2x & = -2 & | :-2 \\x & = 1.
> \end{align*}
> $$


> [!abstract] Vurdering uke 5
> Løs likningen $\frac{4x+10}{3} = 10$. på to måter, formelt og uformelt.

> [!note]- Løsning 
> **Vurderingskriterier**
> Likningen må løses både formelt og uformelt.
> Eksempler: Uformelt kan være å jobbe baklengs. Stegene er
> 1. Ta et ukjent tall og gang med 4.
> 2. Legg til 10
> 3. Del det nye tallet på 3
> 4. Da får vi 10
> 5. Baklengs: Vi har 10 -> Multipliser med 3 (30)-> Trekk fra 10 (20)--> Del på 4 (5).
> 
> Formelt:
> $$
> \begin{align*}
> \frac{4x+10}{3} & = 10 & |\phantom{-}\cdot 3\\ 4x +10 & = 30 & |\phantom{\cdot}-10 \\ 4x & = 20 & | :4 \\x & = 5.
> \end{align*}
> $$
> 


> [!abstract] Vurdering uke 6
> 
> Løs likningen $3(x-1)=2(x+1)$ på to måter, formelt og uformelt.





> [!note]- Løsning 
>  **Vurderingskriterier**
>  
>  *Formelt*:
>  $$
>  \begin{aligned}
>  3(x-1) &= 2(x+1) \\
>  3x - 3 & = 2x + 2 && \bigg|-2x \\
>  3x-2x-3 & = \cancel{ 2x-2x }+2 \\
>  x - 3  &=  2 && \bigg| + 4 \\
>  x &= 5.
>  \end{aligned}
> $$
> 
> *Uformelt*: For eksempel, vi gjetter på $x=0$ og får $3(0-1)=-3$ på venstre side og $2(0+1) = 2$ på høyre side. Vi ser at høyre side stiger med 2 og venstre stiger med 3. Hver gang vi øker $x$ med én, vil forskjellen altså bli én mindre. Siden differansen på venstre og høyre side når $x=0$ er $5$, må vi øke til $x=5$. Vi har derfor løsningen $x=5$.







## Medium 

> [!abstract] 1
> Alfa s. 279
> 1.  3.40 e)--f). Løs likningene på flere måter der det virker naturlig.
> 2. 3.48 a)

> [!abstract] 2
> Løs likningene.
> 
> $\frac{4(x - 1)}{3} - 2x = \frac{3x}{5}$

> [!note]- Løsning 
> $$
> \begin{align*} \frac{4(x-1)}{3} - 2x &= \frac{3x}{5} && \bigg| \cdot(3\cdot5) \\ \cancel{ 3 }\cdot5\cdot \frac{4x-4}{\cancel{ 3 }} - 3\cdot5\cdot2x & = 3\cdot\cancel{ 5  } \cdot\frac{3x}{\cancel{ 5 }} \\ 20x-20-30x &= 9x \\ -10x-20 & = 9x && \bigg| +10x \\ -20 &= 19 x \\ x = \frac{-20}{19}
> \end{align*} 
> $$

> [!abstract] 2.5
> Løs likningen.
> 
> $\frac{5}{2}(2x + 1) = \frac{x - 1}{3} + 7$

> [!note]- Løsning 
> $$
> \begin{align*} 
> \frac{5}{2}(2x+1) & = \frac{x-1}{3}+7  && \bigg|\cdot2
> \\
> 5(2x+1) &= \frac{2x-2}{3}+14 && \bigg| \cdot3
> \\
> 3(10x+5) &= 2x-2+3\cdot14 
> \\
> 30x+15 &= 2x-2+42 && \bigg|-15
> \\
> 30x &= 2x+25 && \bigg| -2x
> \\
> 28x &= 25
> \\
> x &= \frac{25}{28}
> \end{align*} 
> $$



> [!abstract] 3
> Løs likningen på tre måter: ved hjelp av strategiene _holde over_ og _arbeide baklengs,_ og på «vanlig» måte.
> $$
> \frac{20}{\left( \frac{4x - 2}{3} \right) - 2} = 5
> $$


> [!abstract] 4
> På et klassetrinn er det $70$ elever. Alle deltar i en idrett. Antallet elever på håndball er tre mer enn det dobbelte av antallet på fotball. De som spiller basket, er tre færre enn halvparten av dem som spiller fotball. Ingen elever deltar på mer enn én idrett.
> 1. Finn ut hvor mange som spiller hva ved hjelp av en illustrasjon.
> 2. Sett opp og løs en likning som passer situasjonen. Vis tydelig hvordan likningen samsvarer med situasjonen.


> [!abstract] 5
> Blant 163 spurte mennesker, svarte alle at de spiser enten taco, nudler eller foie gras på fredager. Det er 35 flere som spiser taco enn nudler, og antallet som spiser foie gras er det tredobbelte av én mer enn antallet som spiser nudler.
> 1.  Fullfør og bruk illustrasjonen under til å finne ut hvor mange som spiser hva.
> 2. Sett opp og løs likningen situasjonen svarer til.
> 
> ![](https://raw.githubusercontent.com/Andremartiny/MA-173/main/img/2023-03-24-15-14-26.png)




> [!abstract] Vurdering uke 3
> 1. Løs likningen $\displaystyle\frac{14}{3-\frac{4x-1}{3}}= 7$ på en uformell måte. Løs likningen deretter formelt.
> 2. På en skole kan elevene velge én idrett. Det er 26 flere i svømming enn i turn, og det er dobbelt så mange i klatring som i svømming. I alt er det 370 elever på skolen. Hvor mange er det elever er det på de tre idrettsgrenene? Løs problemet på to måter.

> [!note]- Løsning 
> 1. Besvarelsen skal løses uformelt. For eksempel gir hold over metoden at nevner må være 2. Som igjen betyr at $\frac{{4x-1}}{3}= 1$.... og så videre... $x=1$. Deretter må likningen løses formelt.
> 2. Besvarelsen må løse på to ulike måter. For eksempel kan det merkes at det er svømming som det sammenliknes med, så vi lar $x$ være antall elever som går på svømming. Da er det $x-26$ som gir antall som turner og $2x$ som klatrer. Totalt skal dette bli $370$, altså at $x-26+x+2x = 370$. Med litt omforming gir dette $4x = 396$ eller $x=99$. Altså at $99$ svømmer, $198$ klatrer og $73$ turner. Det går også å gjøre noe strukturert gjetting. Gjetter vi at det er 30 som turner må det være 56 som svømmer og dermed 112 som klatrer. Totalt 198. Vi ser altså at det gir 172 elever for lite. Hver gang vi øker turnere med én får vi like stor økning i svømmere, og dobbelt økning i klatrere. En totalt økning på fire elever per ekstra elev som turner. Øker vi turnere med 40 øker vi totalt med 160 elever. Vi skal ha opp til 172, så vi må ha tre elever til, altså øke fra $30$ elever til $73$. Nå får vi samme løsning som over.



> [!abstract] Vurdering uke 4
> 1. Løs likningen på to måter, formelt og uformelt. La det gå klart frem at hvert steg i løsningen din må være riktig. $$3 + \frac{12}{\frac{3x+3}{8}} = 7$$
> 2. En lærer ga elevene følgende tallgåte: "Tenk på et tall. Legg til fem, og multipliser summen med fire. Trekk så fra seks, og del det du fikk på to. Trekk til slutt fra syv. Hvis du forteller meg tallet du nå har, skal jeg fortelle deg hvilket tall du tenkte på." En elev oppgir åtte. Hvilket tall tenkte hun på? Løs problemet på to måter.  



> [!note]- Løsning 
> Vurderingskriterier
> 1. Løses på to måter, formelt (vanlig) og uformelt (for eksempel «holde over»). 3 + noe er 7, noe er 4 og så videre.
> 2. Studenten må løse problemet på to måter.
>     1. For eksempel kan de jobbe baklengs. De endte med åtte. Dette fikk de ved å trekke fra syv. Da hadde de altså $15$. Tallet $15$ kom fra å dele på to, så da hadde de $30$. Igjen kom $30$ fra en subtraksjon på $6$, så de hadde $36$ på det tidspunktet. Siden $36$ kom fra å multiplisere med $4$ kan vi gå baklengs ved å ta $\frac{36}{4} = 9$. Tallet $9$ fikk de ved å legge til fem, så vi finner det originale tallet ved å trekke fra fem. De må altså ha begynt med $4$.
>     2. En annen måte vil være algebraisk. Da kan vi skrive opp hele rekken. Vi kaller tallet eleven tenke på for $a$. Gjennom forklaringen får vi vite at $$\frac{(a+5)\cdot 4 - 6}{2}-7 = 8.$$Løser de denne ligningen vil de essensielt gjøre det samme som er gjort over (eller noe tilsvarende).

> [!abstract] Vurdering uke 5
> 1. Løs likningen $\displaystyle\frac{15}{\frac{2x-4}{2}-2}= 5$ på en uformell måte. Løs likningen deretter formelt.
> 2. Henrik og André gikk for å handle. Henrik hadde med seg 44 kr og André hadde 98 kr. André brukte fire ganger så mye som Henrik. Da de gikk hjemover, hadde de like mye igjen. Hvor mye brukte hver av dem? Løs problemet på to måter.


> [!note]- Løsning 
>  **Vurderingskriterier**
>  1. Løses _både_ formelt og uformet (for eksempel holde over). 15 delt på noe er 5, noe er 3 --> noe minus 2 er 3, noe er 5 --> osv.
>  2. Besvarelsen må løse problemet på to måter.
>      1. For eksempel kan det gjettes litt fornuftig. Dersom Henrik brukte 20 kroner brukte André 80 kroner. Da ville Henrik hatt 24 kroner igjen og André 18. Vi prøver noe mindre. Henrik bruker 15 gir at andre bruker 60. Henrik har da 29 kroner igjen og Andre har 38 igjen. Vi gikk ned for mye og differansen ble _større_. Prøver noe nærmere 20, for eksempel at Henrik bruker 18, da bruker André 72. Henrik har da 26 kroner igjen og Andre har 26 kroner igjen og vi har funnet løsningen.
>      2. Det går an å legge inn litt logikk i oppgaven over. Vi ser at for hver krone Henrik bruker, bruker André tre mer. Siden André satt igjen med 6 kroner mindre ved første gjett, vet vi at differansen mellom dem ville vært 6 kroner mindre dersom Henrik brukte 2 kroner mindre, altså 18.
>      3. Formelt: La $x$ være kronene Henrik bruker. Da bruker André $4x$. Det de står igjen med er $44-x$ og $98-4x$. Siden dette er likt får vi likningen $44-x = 98-4x$ eller $3x = 54$ som gir $x = 18$.

> [!abstract] Vurdering uke 6
> 1. Løs likningen $\displaystyle\frac{20}{\frac{4x-2}{3}-2}= 5$ på en måte som ikke innebærer "flytt og bytt" eller å utføre samme operasjon på hver side av likhetstegnet en uformell måte. Løs likningen deretter formelt.
> 2. På et klassetrinn er det $70$ elever. Alle deltar i en idrett. Antallet elever på håndball er tre mer enn det dobbelte av antallet på fotball. De som spiller basket er tre færre enn halvparten av de som spiller fotball. Ingen elever deltar på mer enn én idrett.
>     1. Finn ut hvor mange som spiller hva ved hjelp av en illustrasjon.
>     2. Sett opp og løs en likning som passer situasjonen. Vis tydelig hvordan likningen samsvarer med situasjonen.


> [!note]- Løsning 
>  **Vurderingskriterier**
>  1. Vi begynner uformelt
>      - For eksempel kan vi bruke hold over metoden. $\displaystyle\frac{20}{\square} = 5$ betyr at $\square = 4$. Ser vi videre på nevneren som må være 4, får vi $\triangle-2 = 4$, altså at $\triangle = 6$. Med andre ord er $\dfrac{4x-2}{3}=6$. Holder vi over telleren ser vi at $4x-2 = 18$. Holder vi over $4x$ ser vi at $4x$ må være $20$, eller at $x = 5$.
>      - Formelt kan løses slik
>    
>  $$
>  \begin{aligned}
>  \frac{20}{\frac{4x-2}{3}-2} &= 5 && \quad \quad\quad \bigg| \left( \frac{4x-2}{3}-2 \right) \\
>  \cancel{ \left( \frac{4x-2}{3}-2 \right) } \cdot \frac{20}{\cancel{ \frac{4x-2}{3}-2 }} & = \left( \frac{4x-2}{3}-2 \right)\cdot 5 \\ 20 &= \frac{20x-10}{3}-10 && \quad \quad\quad \bigg|\cdot 3 \\ 60 &= 20x-10-30 \\ 60 & = 20x-40 && \quad \quad\quad \bigg|+40 \\ 100 & = 20x \\ 5 = \frac{100}{20}&=  x.
>  \end{aligned}
>  $$
>  2. Vi ser at fotball refereres til i alle utsagn. Vi bruker dette som referanse.
>      1. Vi kan ta utgangspunkt i fotball og la røde streker være minus 1 og blå være positiv 1. Da vil blå mot rød kanselleres. Da kan man illustrere slik. <br>        ![400](https://raw.githubusercontent.com/Andremartiny/MA-173/93be951e868b7f11424afd2051c9ac36f5d76208/img/likn/Untitled%20Diagram-Page-1.drawio.svg) <br>        Første linje viser sammenhengen vi blir fortalt. En blokk som spiller fotball, så en blokk som er håndball, bestående av to grupper som er like stor som fotballgruppen + 3. Neste gruppe er basketgruppen, der er det en blokk som tilsvarer halvparten av fotballgruppen, samt tre negative som illustrerer at det er tre mindre enn halvparten.       Overgangen til neste er bare kansellering mellom de tre flere i håndballgruppen og tre færre i fotballgruppen. <br> Tredje linje er en dobling av situasjonen, som gir totalt 140 elever. Siste linje er de to halve blokkene satt sammen til en hel, og vi ser nå enkelt at 7 fotballgrupper tilsvarer 140, eller at fotballgruppen er 20. Nå kan vi kikke på første linje å si at det er 40+3 som spiller håndball og 10-3=7 som spiller basket.
>      2. Ser vi på øverste linje blir likningen og regningen slik
> $$
> \begin{aligned}
> x+(2x+3)+\left( \frac{x}{2}-3 \right) &= 70 \\
> x+2x+3+ \frac{x}{2}-3 & = 70 && \quad \quad\quad \bigg| \cdot2  \\
> 2x+4x+\cancel{ 6 }+x\cancel{ -6 } &= 140  \\ 7x &= 140 \\ x  &= 20.
> \end{aligned}
> $$















<br>
